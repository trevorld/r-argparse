...
\usepackage{listings}
\title{argparse Command Line Argument Parsing}
\begin{document}
\maketitle
...
setwd(system.file("exec", package="argparse")) # to access the "Rscript files"
list_file_command <- "ls"
chmod_command <- "chmod ug+x display_file.R example.R"
path_command <- "export PATH=$PATH:`pwd`"
@

In our working directory we have two example R scripts, named ``example.R'' and ``display\_file.R'' illustrating the use of the argparse package.
\newline\newline
...
By default \emph{argparse} will generate a help message if it encounters \verb|--help| or \verb|-h| on the command line.  Note how \verb|%(default)s| in the example program was replaced by the actual default values in the help statement that \emph{argparse} generated.  
\newline\newline
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
...
If you specify default values when creating your \verb|ArgumentParser| then \emph{argparse} will use them as expected.
\newline\newline
\noindent\verb|\Sexpr{paste("bash$", command)}|
<<echo=FALSE>>=
cat(system(sprintf("%s %s 2>&1", Rscript_executable, command),  intern=TRUE), sep="\n")   
